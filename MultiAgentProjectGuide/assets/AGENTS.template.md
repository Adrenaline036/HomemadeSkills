...
- Freeze candidate identity and bind it to the minimal privacy-screened dynamic review package. Keep stable project context byte-identical within a cache generation; if it changes, warm the new context once before review.
...
- Use the dedicated DeepSeek review MCP rather than a project-specific serializer. Supply explicit obligation IDs/clauses and require a clean finish, structurally and semantically valid compact JSON, exact package id, coherent verdict/findings/unknowns, obligation-bound unique root causes, and closed Provider usage. Empty, truncated, duplicate/no-op or invented-requirement findings, contradiction/no-defect self-talk, non-clean-finish, or invalid usage fails closed. Valid JSON alone is not a valid review.
